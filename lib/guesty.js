...
async function getToken() {
  ...
  if (!resp.ok) {
    ...
    console.error('Token error:', resp.status, errText);
    throw new Error('Open API token error: ' + resp.status + ' ' + errText);
  }
...
export async function guestyFetch(url, options, retried) {
  if (!options) options = {};
